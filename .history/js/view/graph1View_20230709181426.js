...
class Graph1View extends View {
  ...
  generateMarkUp(data) {
    ...
    // Calculate the filtered data range for scaling
    const filteredDataRange = Math.max(...data.map((el) => el.amount)) - Math.min(...data.map((el) => el.amount));
  
    // Calculate the yScale based on the filtered data range
    const yScale = (this.height - this.padding * 2) / filteredDataRange;
  
    // Calculate the xScale based on the number of data points
    ...
    let svgMarkup = `
      <svg xmlns="http://www.w3.org/2000/svg" width="${this.width}" height="${this.height}">
        <line x1="${this.padding}" y1="${this.height - this.padding}" x2="${this.width - this.padding}" y2="${this.height - this.padding}" stroke="none" />
        <line x1="${this.padding}" y1="${this.height - this.padding}" x2="${this.padding}" y2="${this.padding}" stroke="none" />
        <polyline points="${data.map((el, i) => `${this.padding + i * xScale},${this.height - (this.padding + (el.amount - Math.min(...data.map((el) => el.amount)))) * yScale}`).join(" ")}" fill="none" stroke="#164d71" stroke-width="2" />`;
    ...
    data.forEach((el, i) => {
      ...
      svgMarkup += `
        <circle
          ...
          cy="${this.height - (this.padding + (el.amount - Math.min(...data.map((el) => el.amount)))) * yScale}"
          r="${dotRadius}"
          fill="#3498db"
          class="data-point"
