...
class Graph1View extends View {
  ...
  generateMarkUp(data) {
    ...
    // Add text labels for the first and last entries
    const firstEntry = data[0];
    const lastEntry = data[data.length - 1];

    svgMarkup += `
      <text x="${this.padding + xScale / 2}" y="${this.height - this.padding - 10}" fill="#fff" font-size="12px" text-anchor="middle">${firstEntry.amount} XP (${firstEntry.date})</text>
      <text x="${this.padding + (data.length - 1) * xScale}" y="${this.height - this.padding - 10}" fill="#fff" font-size="12px" text-anchor="end">${lastEntry.amount} XP (${lastEntry.date})</text>
    `;
