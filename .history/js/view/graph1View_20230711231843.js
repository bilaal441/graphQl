          ...
          .map(
            (el, i) =>
              `${this.padding + i * xScale},${
                ...
                (this.padding +
                  (el.amount -
                    Math.min(...data.map((el, i) => el.amount)) * yScale))
              }`
          )
          .join(" ")}" fill="none" stroke="#164d71" stroke-width="2" />`;

    let prevAmount = Math.min(...data.map((el) => el.amount));
    data.forEach((el, i) => {
      const dotRadius = 2;
      const currentAmount =
        el.amount - Math.min(...data.map((el) => el.amount));
      const progressionAmount = currentAmount + prevAmount;

      svgMarkup += `
        <circle
          cx="${this.padding + i * xScale}"
          cy="${this.height - (this.padding + progressionAmount * yScale)}"
          r="${dotRadius}"
          fill="#3498db"
          class="data-point"
          data-amount="${Math.round(el.amount / 1000)}"
