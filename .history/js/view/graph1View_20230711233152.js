...
class Graph1View extends View {
  ...
  // const

   {
    ...
    data.forEach((el, i) => {
      cumulativeTotal += el.amount;

      const dotRadius = 2;
      const dotY =
        this.height -
        (this.padding +
          (cumulativeTotal - Math.min(...data.map((el) => el.amount)))) *
          yScale;

      svgMarkup += `
        <line x1="${this.padding + i * xScale}" y1="${dotY}" x2="${
        this.padding + (i + 1) * xScale
      }" y2="${dotY}" stroke="#164d71" stroke-width="2" />
        <circle cx="${
          this.padding + i * xScale
        }" cy="${dotY}" r="${dotRadius}" fill="#3498db" class="data-point" data-amount="${Math.round(
