...
class Graph1View extends View {
  ...
  setHeight() {
    const parentWidth = parseInt(getComputedStyle(this.parentEl).width);
    const aspectRatio = 0.75; // Adjust this value according to your desired aspect ratio

    const maxHeight = parentWidth;
    const calculatedHeight = parentWidth * aspectRatio;

    this.height = Math.min(maxHeight, calculatedHeight);
  }
