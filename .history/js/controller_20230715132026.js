...
const controlUpdateUI = () => {
  ...
  if (state.islogin) {
    ...
    const skills = [
      {
        amount: 27,
        createdAt: "2023-02-22T15:17:25.055501+00:00",
        id: 60529,
        objectId: 101225,
        path: "/london/div-01/color",
        type: "skill_go"
      },
      // Add more skill objects if needed
    ];
    ...
    skills.forEach(skill => {
      ...
      path.setAttribute("d", `M${centerX},${centerY} L${startX},${startY} A${radius},${radius} 0 ${percentage > 0.5 ? "1" : "0"},1 ${endX},${endY} Z`);
      ...
      // Add skill labels
      const labelX = centerX + (radius * 0.6) * Math.cos(startAngle + (endAngle - startAngle) / 2);
      const labelY = centerY + (radius * 0.6) * Math.sin(startAngle + (endAngle - startAngle) / 2);
