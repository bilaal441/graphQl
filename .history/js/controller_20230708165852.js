...
window.addEventListener("load", () => {
  ...
  if (state.islogin) {
    ...
    const yScale =
      (height - padding * 2) / (Math.max(...data.map(el => el.amount)) - Math.min(...data.map(el => el.amount)));
    ...
    let svgMarkup = `
      ...
      height - padding
        ...
        <polyline points="${data.map(el => el.amount)
          .map(
            (d, i) =>
              `${padding + i * xScale},${height - (padding + d * yScale)}`
          )
