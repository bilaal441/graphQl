...
export async function fetchData(query= q) {
...
export const formatSection1Data = () => {
  const level = Math.round(
    state.data.result
      .filter(
        (el) =>
          el.path.includes("/london/div-01") &&
          !el.path.includes("piscine-js/") &&
          !el.path.includes("piscine-js-2weeks") &&
          el.type === "user_audit"
      )
      .reduce((occ, curr) => occ + curr.grade, 0)
  );
  ...
  const xp = Math.round(
    state.data.transaction
      ...
      .reduce((occ, curr) => occ + curr.amount, 0) / 1000
  );
  console.log(
    state.data.progress.filter(
      (el) =>
        el.path.includes("/london/div-01") &&
        !el.path.includes("piscine-js/") &&
        !el.path.includes("piscine-js-2weeks")
